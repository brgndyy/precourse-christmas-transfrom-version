...
- 주문 메뉴를 직접 입력받는 것이 아니라, 사용할 수 있는 금액을 입력받는다. 입력은 1000원 단위여야하며, 최소 메인메뉴 가격인 25000원 이상이어야한다.

- 해당 금액에 맞도록 메뉴를 추천해준다.
  메뉴를 추천해줄때는 무조건 메인메뉴는 최소 1개가 들어가야하며, 중복 되는 메뉴는 없어야한다.

- 더이상 메뉴를 추천해줄수 없을때, 거스름돈과 함께 추천 메뉴 리스트를 추천해준다.

- 총 주문내역에서 기존 미션 조건과 동일하게 할인 혜택을 총합하여 출력한다.
